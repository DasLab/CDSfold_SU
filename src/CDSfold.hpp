		    	...
		    	for (q = j-1; q >= minq; q--) {
				    for (unsigned int Rq = 0; Rq < pos2nuc[q].size() ; Rq++) {
					    ...
					    for (int const Li1_nuc : pos2nuc[i+1]) {
					    	if(NCflg == 1 && i2r[Li1_nuc] != NucConst[i+1]){continue;}
					    	if(DEPflg && Dep1[ii2r[Li_nuc*10+Li1_nuc]][i] == 0){ continue;} // dependency between i and i+1
					    	if(i+1 == p && Li1_nuc != Lp_nuc){ continue; } // In the case of i and p, the base of i + 1 and the base of p must match. (1)

				    		for (int const Rj1_nuc : pos2nuc[j-1]) {
						    	if(NCflg == 1 && i2r[Rj1_nuc] != NucConst[j-1]){continue;}
						    	if(DEPflg && Dep1[ii2r[Rj1_nuc*10+Rj_nuc]][j-1] == 0){ continue;} // dependency between j-1 and j
					    		if(q == j - 1 && Rj1_nuc != Rq_nuc){ continue;} // In the case of q and j, the base of q and the base of j-1 must match. (2)
						    	...
						    	for (int const Lp1_nuc : pos2nuc[p-1]) {
							    	...
							    	if(i == p-1 && Li_nuc != Lp1_nuc){ continue; }   // In the case of i and p, the base of i and the base of p-1 must match. The reverse of (1)
						    		if(i == p-2 && Li1_nuc != Lp1_nuc){ continue; }  // In the case of i, X, p, the base of i + 1 and the base of p-1 (X) must match.
